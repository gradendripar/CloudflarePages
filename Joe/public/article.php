<?php 
    ...
    if($this->options->JsdelivrPath == 'on') {
        $JsdelivrPath = $this->options->themeUrl;
...
<article class="joe_detail__article">
    <?php if (strpos($_SERVER['HTTP_HOST'], 'zhinian') !== false) : ?>
        <!-- 横向自适应 -->
        <div id="google_ad_article" style="width:85%">
            <span class="google_ad_suoming" style="margin: auto;">点击广告即可查看隐藏内容</span>
    ...
    <?php if (!$this->hidden && $this->fields->video) : ?>
        <div class="joe_detail__article-video">
            <div class="play">
                ...
                <div class="box">
                    <iframe allowfullscreen="true" data-player="<?php $this->options->JCustomPlayer ? $this->options->JCustomPlayer() : Helper::options()->themeUrl('library/player.php?JsdelivrPath='.$JsdelivrPath.'&url=') ?>"></iframe>
                </div>
            </div>
            <div class="episodes">
    ...
    <?php if ($this->is('post')) : ?>
        ...
        <?php else : ?>
            <?php 
                $this->content = CreateCatalog($this);
