...
function _getCityById($ip) {
    if(Helper::options()->CommentCity == 'on') {
...
function _getThumbnails($item)
{
	...
	if (sizeof($result) < 3) {
		...
		if ($custom_thumbnail) {
			...
			for ($i = 0; $i < 3; $i++) {
				$result[] = $JsdelivrPath.'/npm/typecho-joe-next@6.0.0/assets/thumb/' . rand(1, 42) . '.jpg';
			}
		}
	}
	return $result;
...
// 实现自定义CDN文件域名
function _CdnUrl4Themes($path = null, $type = true)
{
    if (empty(Helper::options()->CdnUrl4Themes)) {
		if ($path) {
			$path = _UrlVersion($path);
		}
		return Helper::options()->themeUrl($path);
	}
	$url = Helper::options()->CdnUrl4Themes . $path;
	if ($path) {
		$url = _UrlVersion($url);
	}
    if ($type == false) {
