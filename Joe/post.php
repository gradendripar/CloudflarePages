...
<head>
    ...
    <?php else : ?>
        <link rel="stylesheet" href=<?php echo $JsdelivrPath ?>/npm/prismjs@1.23.0/themes/prism.min.css>
    <?php endif; ?>
    <script src=<?php echo $JsdelivrPath ?>/npm/clipboard@2.0.6/dist/clipboard.min.js></script>
    <script src=<?php echo $JsdelivrPath ?>/npm/typecho-joe-next@6.2.4/plugin/prism/prism.min.js></script>
    
    <link rel="stylesheet" href="<?php _CdnUrl4Themes('assets/css/joe.post.min.css'); ?>">
    <script src="<?php _CdnUrl4Themes('assets/js/joe.post_page.js'); ?>"></script>
    ...
    <div id="Joe">
        ...
        <div class="joe_container">
            <div class="joe_main joe_post">
                <?php if ($this->options->ArticleDirTree !== 'off') : ?>
                    <?php GetCatalog($this); ?>
                <?php endif; ?>
                <div class="joe_detail" data-cid="<?php echo $this->cid ?>">
                    ...
                    <div class="joe_post__ad">
                        <?php
                            $JADPost4Header_arr = [];
                            $JADPost4Header_text = $this->options->JADPost4Header;
                            if ($JADPost4Header_text) {
                            $JADPost4Header_arr = explode("\r\n", $JADPost4Header_text);
                            if (count($JADPost4Header_arr) > 0) {
                                for ($i = 0; $i < count($JADPost4Header_arr); $i++) {
                                    $img = explode("||", $JADPost4Header_arr[$i])[0];
                                    $url = explode("||", $JADPost4Header_arr[$i])[1];
                                    $mdate = explode("||", $JADPost4Header_arr[$i])[2];
                                    
                                    if(!empty(trim($mdate))) {
                                        $nowDate = date("Y-m-d");
                                        if(trim($mdate) >= $nowDate) {
                    ...
                    <div class="joe_post__ad" style="margin-bottom: 15px;">
                        <?php
                            $JADPost4Fotter_arr = [];
                            $JADPost4Fotter_text = $this->options->JADPost4Fotter;
                            if ($JADPost4Fotter_text) {
                            $JADPost4Fotter_arr = explode("\r\n", $JADPost4Fotter_text);
                            if (count($JADPost4Fotter_arr) > 0) {
                                for ($i = 0; $i < count($JADPost4Fotter_arr); $i++) {
                                    $img = explode("||", $JADPost4Fotter_arr[$i])[0];
                                    $url = explode("||", $JADPost4Fotter_arr[$i])[1];
                                    $mdate = explode("||", $JADPost4Fotter_arr[$i])[2];
                                    
                                    if(!empty(trim($mdate))) {
                                        $nowDate = date("Y-m-d");
                                        if(trim($mdate) >= $nowDate) {
                                            $JADPost4Fotter[] = array("img" => trim($img), "url" => trim($url));
                                        }
                                    }
                                    else {
                                        $JADPost4Fotter[] = array("img" => trim($img), "url" => trim($url));
                        ...
                        ?>
                        <?php if (@sizeof($JADPost4Fotter) > 0) : ?>
                            <div class="swiper-container">
                                <div class="swiper-wrapper">
                                    <?php foreach ($JADPost4Fotter as $item) : ?>
                                    <div class="swiper-slide">
                                        <a class="item" href="<?php echo $item['url'] ?>" target="_blank" rel="noopener noreferrer nofollow">
                                            <img width="100%" class="image lazyload" src="<?php _getLazyload() ?>" data-src="<?php echo $item['img'] ?>" />
                                        </a>
...
<?php if ($this->options->ArticleDirTree !== 'off') : ?>
    <script src="<?php _CdnUrl4Themes('assets/js/jfloor.js'); ?>"></script>
<?php endif; ?>